...
- Isolated Personal Resource Display from regular nameplate behavior so it shows only DragonUI health and resource bars, with optional camera-scale stabilization while remaining attached to the character plus independent visibility, position, scale, dimensions, horizontal/vertical layout, value-format, and absorb-shield controls.
